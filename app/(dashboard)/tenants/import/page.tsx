...
const SECTION_ORDER: TenantImportSystemField["section"][] = [
  "tenant",
  "owner",
  "property",
  "rental",
...
const SECTION_LABELS: Record<TenantImportSystemField["section"], string> = {
  tenant: "Tenant Personal Fields",
  owner: "Owner Fields",
  property: "Property Fields",
  rental: "Rental / Agreement Fields",
  cheque: "Cheque Schedule (1–10)",
  system: "System / Migration Fields",
};
...
function ImportContent() {
    ...
    return (
      <div className="flex min-h-[40vh] flex-col items-center justify-center text-center">
        ...
        <p className="mt-1 text-xs text-slate-400">
          Only Master users can access the Tenant Import feature.
        </p>
      </div>
    );
  }
  ...
  const [preview, setPreview] = useState<TenantImportPreviewResult | null>(null);
  ...
  async function handlePreview() {
    ...
    try {
      const data = await tenantsService.importPreview(file);
      setPreview(data);
      // Use the server-computed suggested mapping (alias-aware, covers Zoho CSV
      // column names like "Tenant Name", "From", "To", "Cheque 1", etc.)
  ...
  async function handleImport() {
    ...
    setImporting(true);
    try {
      const data = await tenantsService.importTenants(file, mapping);
      setResult(data);
      setStep("result");
    } catch (err: any) {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle="Upload a Zoho CRM CSV export to import tenants, owners, properties, and rental agreements"
        actions={
          <Link
            href="/tenants"
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="space-y-1.5 text-xs text-blue-800">
                  ...
                  <ul className="list-disc space-y-1 pl-4">
                    <li>Upload your Zoho Tenants CSV export (or any CSV/XLSX with tenant data).</li>
                    <li>Map each column in your file to the appropriate CRM field.</li>
                    <li><strong>Owners</strong> are deduplicated by mobile number — existing owners are updated, new ones are created.</li>
                    <li><strong>Properties</strong> are added as Manual Properties linked to each owner.</li>
                    <li><strong>Tenants</strong> are created with full rental agreement details (rent, deposit, commission, cheques) and linked to the owner + property.</li>
                    <li><strong>Cheques 1–10</strong> are imported as a cheque schedule for each tenant.</li>
                    <li>The Zoho <strong>Record ID</strong> is stored for migration traceability (re-imports update existing records instead of duplicating).</li>
          ...
          <Card>
            ...
            <CardBody className="space-y-4">
              {SECTION_ORDER.map((section) => {
                const fields = preview.systemFields.filter((sf) => sf.section === section);
          ...
          <CardBody className="space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Your tenant data has been processed.</p>
              </div>
            </div>

            {/* Summary stats */}
            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
              <StatCard label="Tenants Created" value={result.tenantsCreated} color="emerald" />
              <StatCard label="Tenants Updated" value={result.tenantsUpdated} color="blue" />
              <StatCard label="Cheques Created" value={result.chequesCreated} color="purple" />
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
              ...
              <StatCard label="Props Skipped (dup)" value={result.propertiesSkipped} color="amber" />
            </div>
            ...
            <div className="flex items-center gap-3">
              <Button onClick={() => router.push("/tenants")}>View Tenants</Button>
...
function FieldGroup({
  ...
  title: string;
  fields: TenantImportSystemField[];
  headers: string[];
