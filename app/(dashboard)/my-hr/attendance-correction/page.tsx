...
import { useRegularizationList, useApplyRegularizationWithAttachment, type RegularizationApplyFormData } from "@/hooks/useHrms";
    ...
    try {
      const payload: RegularizationApplyFormData = {
        ...
        requestedCheckIn: requestedCheckIn
          ? new Date(`${date}T${requestedCheckIn}`).toISOString()
          : undefined,
        requestedCheckOut: requestedCheckOut
          ? new Date(`${date}T${requestedCheckOut}`).toISOString()
          : undefined,
        reason,
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={prefill?.currentStatus?.replace("_", " ") ?? "—"}
                readOnly
          ...
          <div>
            ...
            {hintMap[requestType] && (
              <p className="mt-1 text-xs text-slate-400">{hintMap[requestType]}</p>
            )}
          </div>
...
function DetailDrawer({ req, onClose }: { req: AttendanceRegularization; onClose: () => void }) {
  return (
      ...
      <div className="relative z-10 flex h-full w-full max-w-md flex-col bg-white shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><p className="text-xs text-slate-500">Corrected Check-in</p><p className="font-medium">{formatTime(req.requestedCheckIn)}</p></div>
            <div><p className="text-xs text-slate-500">Corrected Check-out</p><p className="font-medium">{formatTime(req.requestedCheckOut)}</p></div>
          </div>
...
export default function MyAttendanceCorrectionPage() {
  ...
  return (
    <AccessGuard module="my_hr" page="attendance_regularization">
      <div className="space-y-6">
        <PageHeader
          title="Attendance Correction"
          subtitle="Request and track attendance corrections"
          actions={
        ...
        <DataTable<AttendanceRegularization>
          ...
          emptyMessage="You haven't submitted any correction requests yet."
        />
        ...
        {detailReq && (
          <DetailDrawer req={detailReq} onClose={() => setDetailReq(null)} />
        )}
      </div>
    </AccessGuard>
  );
